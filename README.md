<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0f0c29,40:302b63,100:7c3aed&height=220&section=header&text=Morgan%20Reichert&fontSize=52&fontColor=ffffff&fontAlignY=38&desc=Developer%20%E2%80%A2%20AI%20Builder%20%E2%80%A2%20Brussels&descAlignY=60&descColor=c4b5fd&animation=fadeIn" />

</div>

<br/>

<div align="center">

<!-- ╔══════════════════════════════════════════╗ -->
<!-- ║  📸 REMPLACE CE LIEN PAR TON VRAI AVATAR ║ -->
<!-- ║  ex: https://avatars.githubusercontent.com/u/XXXXXX -->
<!-- ╚══════════════════════════════════════════╝ -->
<img src="https://i.postimg.cc/L5tf6L2x/Gemini-Generated-Image-wcryrcwcryrcwcry.png" width="150" height="150" alt="Morgan Reichert" />

<br/><br/>

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&duration=3000&pause=800&color=A78BFA&center=true&vCenter=true&width=620&lines=Salut%2C+moi+c'est+Morgan+%F0%9F%91%8B;D%C3%A9veloppeur+Web+%26+Passionn%C3%A9+d'IA+%F0%9F%A4%96;Bruxelles+%C2%B7+17+ans+%C2%B7+Fondateur+STARIAX;Je+construis+des+apps+%C3%A0+impact+r%C3%A9el+%E2%9C%A8)](https://git.io/typing-svg)

<br/>

[![GitHub](https://img.shields.io/badge/GitHub-Morgan--Reichert-0f0c29?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Morgan-Reichert)
[![STARIAX](https://img.shields.io/badge/Groupe-STARIAX-7c3aed?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Morgan-Reichert?tab=repositories)
[![Bruxelles](https://img.shields.io/badge/📍-Bruxelles%2C+Belgique-302b63?style=for-the-badge)](#)
[![Age](https://img.shields.io/badge/🎂-17+ans-1e1b4b?style=for-the-badge)](#)

</div>

---

## 🙋 À propos de moi

```ts
const morgan = {
  name        : "Morgan Reichert",
  age         : 17,
  location    : "Bruxelles, Belgique 🇧🇪",
  role        : "Développeur Web & Fondateur de STARIAX",
  spécialisé  : ["Web statique", "Intégration IA", "UI/UX soigné"],
  passions    : ["Tech & IA 🤖", "Photographie 📷", "Secourisme 🚑", "Web 🌐"],
  en_ce_moment: "Intégrer l'IA dans chacun de mes projets",
  objectif    : "Créer des apps utiles, accessibles et à impact social",
  disponible  : true,
};
```

> 💡 **Je crois que la technologie doit être au service des gens.**
> Parti du web statique, j'intègre aujourd'hui l'IA dans la totalité de mes projets — pour automatiser, personnaliser, et apporter une vraie valeur à l'utilisateur.
> Chaque app que je crée répond à un besoin réel : sanitaire, sécuritaire, ou simplement alléger la charge du quotidien.

---

## 🛠️ Ma Stack Technique

> ✏️ *Pour ajouter une techno : copie un badge `<img>` et change le `label` et `logo` sur [shields.io](https://shields.io/badges/static-badge)*

### 🌐 Frontend
<p>
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" />
  <!-- ➕ AJOUTE ICI -->
</p>

### ⚙️ Build Tools & Frameworks
<p>
  <img src="https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white" />
  <img src="https://img.shields.io/badge/shadcn/ui-000000?style=for-the-badge&logo=shadcnui&logoColor=white" />
  <!-- ➕ AJOUTE ICI -->
</p>

### 🤖 IA & Automatisation
<p>
  <img src="https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white" />
  <img src="https://img.shields.io/badge/Anthropic_Claude-7c3aed?style=for-the-badge&logoColor=white" />
  <!-- ➕ AJOUTE ICI -->
</p>

### 🗄️ Backend & Base de données
<p>
  <img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/Supabase-3ECF8E?style=for-the-badge&logo=supabase&logoColor=white" />
  <!-- ➕ AJOUTE ICI -->
</p>

### 🧰 Outils & Déploiement
<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  <img src="https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white" />
  <img src="https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white" />
  <!-- ➕ AJOUTE ICI -->
</p>

---

## 🚀 STARIAX — Mes Projets

<div align="center">

> **STARIAX** est mon groupe de développement personnel.
> Une suite d'applications pensées avec soin, avec une philosophie simple : **chaque outil doit avoir un impact réel.**

</div>

<br/>

| Projet | Description | Stack | Statut |
|--------|-------------|-------|--------|
| 🧠 **[MindScope](https://github.com/Morgan-Reichert/mindscope-v1)** | Outil d'introspection & d'analyse de pensée assisté par IA | React · TS · Vite · AI | ![v1.0](https://img.shields.io/badge/v1.0-7c3aed?style=flat-square) |
| 🌙 **[NightWatch](https://github.com/Morgan-Reichert/nightwatch)** | App iOS + Web de veille sociale intelligente | Swift · React · API | ![Beta](https://img.shields.io/badge/Beta-302b63?style=flat-square) |
| ✨ **[Stariax Showcase](https://github.com/Morgan-Reichert/stariax-showcase)** | Site vitrine officiel du groupe STARIAX | React · TS · Tailwind | ![Live](https://img.shields.io/badge/Live-10b981?style=flat-square) |
| ⚡ **Challenger IA** | Plateforme de challenges gamifiés propulsée par l'IA | *En cours…* | ![Soon](https://img.shields.io/badge/Coming_Soon-f59e0b?style=flat-square) |

<br/>

<div align="center">

[![Voir tous les projets STARIAX](https://img.shields.io/badge/→%20Voir%20tous%20les%20projets%20STARIAX-7c3aed?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Morgan-Reichert?tab=repositories)

</div>

---

## 💫 Ce qui me motive

<table>
<tr>
<td width="50%" valign="top">

**🤖 Intelligence Artificielle**

L'IA est un levier concret, pas un buzzword. Je l'intègre dans chaque app pour automatiser les tâches répétitives, personnaliser l'expérience et résoudre des problèmes complexes avec élégance.

</td>
<td width="50%" valign="top">

**🚑 Impact Social**

Mes projets répondent à des besoins réels : sécurité, bien-être, productivité. Passionné de secourisme, je crois que la tech peut sauver du temps — et parfois bien plus.

</td>
</tr>
<tr>
<td width="50%" valign="top">

**📷 Photographie**

L'œil du photographe influence mon code : attention au détail, composition, esthétique. C'est ce qui rend mes interfaces soignées et mes UX vraiment intuitives.

</td>
<td width="50%" valign="top">

**🌍 Web Accessible**

Un web pour tous, pas pour quelques-uns. Je construis des interfaces claires, rapides, pensées pour l'humain — peu importe le device ou le niveau tech.

</td>
</tr>
</table>

---

## 📊 GitHub Stats

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=Morgan-Reichert&show_icons=true&theme=midnight-purple&hide_border=true&bg_color=0f0c29&title_color=a78bfa&text_color=c4b5fd&icon_color=7c3aed&count_private=true" />
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Morgan-Reichert&layout=compact&theme=midnight-purple&hide_border=true&bg_color=0f0c29&title_color=a78bfa&text_color=c4b5fd&langs_count=6" />

<br/><br/>

<img src="https://streak-stats.demolab.com?user=Morgan-Reichert&theme=midnight-purple&hide_border=true&background=0f0c29&ring=7c3aed&fire=a78bfa&currStreakLabel=c4b5fd" />

</div>

---

## 📬 Me contacter

<div align="center">

*Tu es recruteur, collaborateur, ou simplement curieux ? N'hésite pas !*

<br/>

<!-- ✏️ REMPLACE les liens par tes vrais profils -->
[![Email](https://img.shields.io/badge/Email-Envoie_un_mail-7c3aed?style=for-the-badge&logo=gmail&logoColor=white)](mailto:contact@stariax.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Morgan_Reichert-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/morgan-reichert)
[![Portfolio](https://img.shields.io/badge/Portfolio-Stariax_Showcase-302b63?style=for-the-badge&logo=vercel&logoColor=white)](https://github.com/Morgan-Reichert/stariax-showcase)

</div>

---

<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:7c3aed,60:302b63,100:0f0c29&height=120&section=footer" />

*Fait avec 💜 à Bruxelles · STARIAX © 2025*

</div>
